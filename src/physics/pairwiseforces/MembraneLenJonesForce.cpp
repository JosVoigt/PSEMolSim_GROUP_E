...
std::array<double, 3> MembraneLenJonesForce::calculateForce(
  ...
  if (p1.isDirectNeighbour(p2)) {
    ...
    return k * (distance - bondDistance * std::sqrt(2)) / distance *
           (p1.getX() + p2.getX());
    //check if force would be repulsive
  } else if (distance <
             std::pow(2, 1 / 6) * (p1.getSigma() + p2.getSigma()) / 2) {
...
MembraneLenJonesForce::operator std::string() const {
  return "Membrane-LennardJonesForce";
}
