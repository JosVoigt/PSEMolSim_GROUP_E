...
class StoermerVerletTest : public testing::Test {
 protected:
  ...
  // particle mass
  double mass = 1;
  std::shared_ptr<ParticleContainerInterface> pc =
      std::shared_ptr<ParticleContainerInterface>(new ParticleContainer);
...
TEST_F(StoermerVerletTest, testCalcF) {
  ...
  std::shared_ptr<PairwiseForce> method =
      std::shared_ptr<PairwiseForce>(new LennardJonesForce(1, 1));
